...
url_date_pat = re.compile(
    r'[^\d]((?:19|20)\d{2})([\./\-_]?)(1[0-2]|0?[1-9])([\./\-_]?)([1-2][0-9]|3[0-1]|0?[1-9])?[^\d]')
...
def _extractFromURL(url):
    ...
    if m:
        ...
        # Either the numbers have leading 0s if single digit, or the seperators are there.
        # Otherwise we don't extract the date.
        length = len(m.group(0)) - 2
        if len(parts) == 3:
            sep_len = len(m.group(2)) + len(m.group(4))
            if length - sep_len < 8 and sep_len < 2:
                return None
        else:   # len(parts) == 2
            sep_len = len(m.group(2))
            if length - sep_len < 6 and sep_len < 1:
                return None

        if len(parts) < 3:
            # Only has part of the full date (missing day of month), try finding a fuller version in the url
            t = ''.join(parts)
            start = 0
            while True:
                idx = url.find(t, start)
                if idx >= 0:
                    cd = url[idx+len(t):idx+len(t)+2]
                    try:
                        val = int(cd)
                        if val > 0 and val <= 31:
                            parts.append(cd)
        ...
        if len(parts) == 3:
            return parseStrDate('/'.join(parts))
...
if __name__ == '__main__':
    d = extractArticlePublishedDate(
        "http://techcrunch.com/2015/11/30/atlassian-share-price/")
    print(d)
